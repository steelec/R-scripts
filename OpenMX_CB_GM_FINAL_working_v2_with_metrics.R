...
SKIP_EXTRA_PLOTTING =                  TRUE # this also does something :-)
...
orig_bx_dir='/home/chris/Documents/Dropbox/Projects/Working/CoBrA_Lab/TractREC/processing/bx/'
...
all_structures_noFA_thresh_S500="2016_03_HCP_allLabels_volume.csv"
#all_structures_SEJAL_S500='2016_03_HCP_allSibs_SEJAL_all_labels_volume_restr_unrestr.csv'
#df_metric<- read.csv(paste(orig_bx_dir,all_structures_noFA_thresh_S500,sep=""),header=T,stringsAsFactors = FALSE)
#library(plyr)
...
## NEW DATA, with all metrics inside it, not just volumes
...
#metric_file <- all_structures_S500 #default old one with volumes in orig_bx_dir
metric_file <- all_volumes_all_metrics_S500_FA_fname
#metric_file <- all_volumes_all_metrics_S500_MD_fname
#metric_file <- all_volumes_all_metrics_S500_KM_fname
#metric_file <- all_volumes_all_metrics_S500_t1wdivt2w_fname
value_type <- 'volume' #c('volume','mean','median','std')
metric <- "FA" ## XXX you need to set this manually for plot output
...
unrestr='/home/chris/Documents/Dropbox/Projects/Working/CoBrA_Lab/TractREC/source/bx/HCP_S500_unrestricted_hcp_freesurfer.csv'

df_restr=read.csv(restr,header=T)
df_unrestr=read.csv(unrestr,header=T)
df_HCPbx=merge(x=df_restr,y=df_unrestr,by="Subject")

HcpData <-merge(x=df_metric,y=df_HCPbx,by.x="ID",by.y="Subject")

#HcpData<- read.csv(paste(bx_dir,metric_file,sep=""),header=T,stringsAsFactors = FALSE)
...
#HcpData$Zygosity <- NA #XXX recoded to correct for families that only include a single MZ twin
HcpData$Recoded_Zygosity[HcpData$Zygosity=="MZ"] <- 1
HcpData$Recoded_Zygosity[HcpData$Zygosity=="NotMZ"] <- .5
HcpData$Recoded_Zygosity[HcpData$Zygosity=="NotTwin"] <- .5
HcpData$Zygosity<-HcpData$Recoded_Zygosity

##  remove problematic individuals here, BEFORE any filtering, 
# VERY low MD values in CB?
#HcpData <- subset(HcpData,ID != 105014) #including or excluding makes no sig diff to herit estimates
...
## REMOVE SOME INDIVIDUALS HERE IF YOU WANT TO!
...
allVars <- allVars[grep(value_type,allVars)] #select only the value_type that we are interested in
CBVars <-allVars[1:33] #use this to calculate the sum of all CB GM volume
#allVars<-CBVars #also only care about them right now

#HippVars <- allVars[34:length(allVars)] #hipp plus all other non-cb vars from magetbrain
HippVars <- c("label_031_L_CA1_volume","label_131_R_CA1_volume",
              "label_034_L_CA2CA3_volume","label_134_R_CA2CA3_volume",
              "label_033_L_CA4DG_volume","label_133_R_CA4DG_volume",
              "label_035_L_stratum_volume","label_135_R_stratum_volume",
              "label_032_L_subiculum_volume","label_132_R_subiculum_volume")

allVars <- HippVars
HcpData$label_000_Control_Variable<-rowSums(HcpData[,CBVars])
#HcpData$label_000_Control_Variable<-rowSums(HcpData[,allVars[c(1,3,5,7,9)]]) #R HEMI ONLY
#allVars<-c("label_031_L_CA1_volume","label_000_Control_Variable","Height","Weight","MZ_NotMZ_NotTwin","Gender_Txt")
#allVars<-allVars[1:6]
## set your control variable
controlVar_txt<-"label_000_Control_Variable"
#controlVar_txt<-"label_080_ITV_volume"
#allVars <- colnames(HcpData)
#allVars<-c(allVars,"label_000_Control_Variable","Height","Weight","Handedness","MZ_NotMZ_NotTwin","Gender_Txt")
...
if (USE_TWINS_ONLY){
  ...
  twinned_ids <- unique(twinned_ids) #might have double entries with the last one, though it should never get there
  print(length(twinned_ids))
  
  
  HcpData_twin_subset<- subset(HcpData_twin_subset, ID %in% twinned_ids)
  allVars <- unique(c('ID','Age_in_Yrs','Gender','Gender_Txt','Mother_ID','sibling_number2','Zygosity','MZ_NotMZ_NotTwin',allVars)) #don't duplicate
...
if (USE_CONTROL_VAR_LM) {
  ...
  for (theVarName in allVars) {
    if  (is.numeric(HcpData_subset[,theVarName])) { #if it is a numeric, we remove the regressor
...
#first remove non-numeric values that we created, but ONLY from the family setup (since we need them later in the subset)
allVars_forFam <- subset(allVars,!allVars %in% c('Gender_Txt','MZ_NotMZ_NotTwin'))
HcpData_forFam <- HcpData_subset[,allVars_forFam]

#  such that MZ is first (if present), then DZ (if present), then sibs (if present)
fam <- reshape(HcpData_forFam,idvar="Mother_ID",timevar="sibling_number2",direction="wide")
names(fam)<-gsub("[.]","_",names(fam))
fam[is.na(fam)] <- 0 ## set values to zero where there is no data

#remove Mother_ID and sibling_number2 and other unecessary values that will not be analysed in the heritability estimates
allVars <- subset(allVars,!allVars %in% c("Mother_ID","sibling_number2",'ID','Age_in_Yrs','Gender','Gender_Txt','Zygosity','MZ_NotMZ_NotTwin'))
...
allVars_txt<-gsub(paste("_",value_type,sep=""),"",allVars)
...
#   dev.off()
  ...
  if (USE_CONTROL_VAR_LM | USE_CONTROL_VAR_MX){
    barx=barplot(y,beside=T,ylim=c(0,1),main=c(paste(metric,value_type,sep=" "),"Genetic Component",gsub("_"," ",gsub("^.*?_","",gsub("^.*?_","",controlVar_txt)))),col=mycols,las=3) #,col=rainbow(11)
...
#     text(barx,.98,ifelse(res[23:33,"ACE_vs_CE_p"]*33<.05,"*",""),cex=2)
    ...
    barx=barplot(y,beside=T,ylim=c(0,1),main=c(paste(metric,value_type,sep=" "),"Genetic Component"),col=mycols,las=3) #,col=rainbow(11)
    error.bar(barx,y,res[,"A_CI_upper"],res[,"A_CI_lower"])
    text(barx,.98,ifelse(res[,"ACE_vs_CE_p"]*33<.05,"*",""),cex=2) #stars for sig, go at 1st posn when >.05
...
colnames(res_demog) <- c("N","Females", "Males","Mean Age", "Age Range")
...
if (!SKIP_EXTRA_PLOTTING) {
  ...
  colnames(corr_subset) <- gsub(value_type,"",colnames(corr_subset))
...
#   print(ggheatmap)
  ...
  library("cowplot")
  ## correlation matrix of our input numerical values
  MZ <- subset(HcpData_subset,MZ_NotMZ_NotTwin=="MZ") #twins always in the first two sets of cols
  DZ <- subset(HcpData_subset,MZ_NotMZ_NotTwin=="NotMZ")
  MZ <- reshape(MZ,idvar="Mother_ID",timevar="sibling_number2",direction="wide")
  DZ <- reshape(DZ,idvar="Mother_ID",timevar="sibling_number2",direction="wide")
  names(MZ)<-gsub("[.]","_",names(MZ))
  names(DZ)<-gsub("[.]","_",names(DZ))
  row_idx=0
  for (theVarName in allVars) {
    row_idx=row_idx+1
    if (!is.character(HcpData_subset[theVarName][,])) {
